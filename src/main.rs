...
fn install_my_acpi_table(phys_addr: u64) -> uefi::Result<usize> {
    let bs = unsafe {
        uefi_services::system_table()
            .as_ref()
            .boot_services()
    ...
    let acpi_table_pool = MemoryType::RUNTIME_SERVICES_DATA;
    let acpi_table_size = mem::size_of::<MyAcpiTable>();
    let acpi_table = bs.allocate_pool(acpi_table_pool, acpi_table_size)
        .map_err(inspect("allocate_pool (acpi_table)"))
        .ignore_warning()?;
    info!("acpi_table: {:?}", acpi_table);
    ...
    let mut acpi_table_data = MyAcpiTable {
        header: DescriptionHeader {
    ...
    unsafe {
        bs.memmove(
            acpi_table,
            &acpi_table_data as *const MyAcpiTable as *const u8,
            acpi_table_size
        );
    }
...
fn efi_main(handle: Handle, system_table: SystemTable<Boot>) -> uefi::Status {
    ...
    let table_key = install_my_acpi_table(phys_addr)
